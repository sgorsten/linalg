...
* `mat<T,M,N> operator $ (const mat<T,M,N> & a, T b)`
* `mat<T,M,N> operator $ (T a, const mat<T,M,N> & b)`
* `mat<T,M,N> & operator $= (mat<T,M,N> & a, const mat<T,M,N> & b)`
* `mat<T,M,N> & operator $= (mat<T,M,N> & a, T b)`
...
The unary functions `abs`, `floor`, `ceil`, `exp`, `log`, `log10`, `sqrt`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sinh`, `cosh`, `tanh`, and `round` can be called with any vector or matrix type. The result is equivalent to producing a vector or matrix by calling the equivalent function from the `std::` namespace on every element of a vector or matrix.

The binary functions `min`, `max`, `fmod`, `pow`, and `atan2` can be called with any two matching vectors or matrices. The result is equivalent to producing a vector or matrix by calling the equivalent function from the `std::` namespace on componentwise pairs of the vectors or matrices.

* `clamp(a,b,c)` clamps the elements of `a` to the lower bound `b` and the upper bound `c`

The binary functions `equal`, `nequal`, `less`, `greater`, `lequal`, and `gequal` can be called with any two matching vectors or matrices. The result is a vector or matrix of `bool` type formed by comparing the elements of the vectors or matrices with operator `==`, `!=`, `<`, `>`, `<=`, and `>=` respectively.

## Reduction Functions

These functions take a vector or matrix type and return a scalar value.
...
As a heavily templated library, the following conventions are used to avoid confusion

* `T` - any scalar type, such as `int`, `float`, or a user-defined scalar type
* `V` - any instantiated type of template `vec<T,M>` or `mat<T,M,N>`
* `F` - any function object type, typically a binary function object being passed to `fold` or `zip`
* `M` - the number of rows in a vector or matrix
* `N` - the number of colums in a matrix
* `a` - the left hand parameter to a binary function or operator
* `b` - the right hand parameter to a binary function or operator
* `i` - the zero-based index of a row in a vector or matrix
* `j` - the zero-based index of a column in a matrix
* `q` - a parameter which is assumed to hold a quaternion
* `v` - a parameter which is assumed to hold a vector, when used with a quaternion function
